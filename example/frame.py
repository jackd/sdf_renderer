...
eye_in_camera_coords = frame.coordinate_transform(eye, R, t)
...
print('offset in camera coordinates (should be zeros): %s' % str(i))
print('transform_error (rank 3): %s' % str(np.max(np.abs(te))))
print('transform_error (rank 2): %s' % str(np.max(np.abs(te0))))
